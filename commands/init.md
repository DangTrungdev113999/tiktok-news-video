---
description: "One-time setup: install/verify Node+ffmpeg+Remotion deps, then configure output folder + ElevenLabs API key + voice_id for the tiktok-news-video plugin."
argument-hint: ""
---

# Init: TikTok News Video plugin

Invoke the Skill `tiktok-news-video:init`. That skill runs `scripts/init.mjs`
and walks the user through anything it reports as failing.
